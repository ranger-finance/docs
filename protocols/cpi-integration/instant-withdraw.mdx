...
description: CPI instruction for instant withdrawal from zero-waiting-period vaults
---
...
| `vault_asset_idle_ata` | Mutable | No | The vault's idle asset token account |
...
<Info>
The `instant_withdraw_vault` instruction shares the same parameters as [`request_withdraw_vault`](/protocols/cpi-integration/request-withdraw) but does not create a receipt account. The LP burn and asset transfer happen atomically in a single transaction.
</Info>

Full reference implementation: [instant_withdraw_vault.rs](https://github.com/ranger-finance/vault-cpi/blob/main/references/instant_withdraw_vault.rs)
